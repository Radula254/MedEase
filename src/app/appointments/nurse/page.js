...
export default function UsersPage() {
  ...
  useEffect(() => {
    if (profileData?.nurse) {
      ...
      fetch(`/api/bookedAppointments?userId=${userId}&role=nurse`).then((response) => {
  ...
  return (
    <section className="max-w-2xl mx-auto mt-10 mb-20">
      <div className="mt-8">
        {users?.filter(user => user?.userInfo?.consultationPayment === true && !user?.userInfo?.selectedDoctor).length > 0 ? 
          users.filter(user => user?.userInfo?.consultationPayment === true && !user?.userInfo?.selectedDoctor).map((user) => (
