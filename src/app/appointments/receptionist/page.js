...
export default function UsersPage() {
  ...
  <div className="mt-8">
    {users?.filter(user => user?.userInfo?.consultationPayment === false && user?.userInfo?.createdAppointment === true).length > 0 ?
      users.filter(user => user?.userInfo?.consultationPayment === false && user?.userInfo?.createdAppointment === true).map((user) => (
        <div key={user._id} className="bg-gray-100 rounded-lg mb-2 p-1 px-4 flex items-center gap-4">
          ...
          <div>
            <Link className="button" href={"/appointments/receptionist/" + user._id}>
              Book
            </Link>
          </div>
        </div>
